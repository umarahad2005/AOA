...
// Function to decode a Huffman encoded string using the tree
...
int main() {
    ...
    cout << "This is lab 12 task 5 - Decode a Huffman encoded string back to original text" << endl;
    cout << endl;
    ...
    cout << "Encoded bits: " << encodedString << endl;
    cout << "\nDecoding step by step:" << endl;
    
    Node* current = root;
    string path = "";
    for (int i = 0; i < encodedString.length(); i++) {
        path += encodedString[i];
        
        if (encodedString[i] == '0') {
            current = current->left;
            cout << "  Bit '" << encodedString[i] << "' -> Go Left";
        } else {
            current = current->right;
            cout << "  Bit '" << encodedString[i] << "' -> Go Right";
        }
        
        if (current->ch != '\0') {
            cout << " -> Found '" << current->ch << "' (code: " << path << ")" << endl;
            current = root;
            path = "";
        } else {
            cout << endl;
    ...
    if (text == decodedString) {
        cout << "\nSUCCESS: Decoded string matches original!" << endl;
    } else {
        cout << "\nERROR: Decoded string does not match original!" << endl;
    }
